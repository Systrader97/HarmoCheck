...
- Provides a green, local Windows UI. The selected data are processed only on the local computer.

## Semestral data handling

Use an explicit `Semester` / `반기` column whenever a workbook contains more than one period. Values such as `H1`, `H2`, `상반기`, and `하반기` are supported. For a file containing only one three-material semester, select its semester in the UI. If a single year has six materials and no semester column, HarmoCheck assigns the first three material codes to H1 and the next three to H2.
