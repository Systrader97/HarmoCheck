...
from .core import AnalysisResult, DEFAULT_TEA, HARMONIZATION_LEVELS, TEaThreshold, analyze_workbook


class HarmoCheckApp(tk.Tk):
    """Green-themed Tkinter application designed for routine EQA assessment."""
    ...
    def __init__(self) -> None:
        ...
        self.default_semester = tk.StringVar(value="H1 (상반기)")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(header, text="KEQAS EQA raw data 기반 반기별 Harmonization 평가 | 3 PT materials per semester", style="Subtitle.TLabel").pack(anchor="w", pady=(4, 0))
        ...
        ttk.Label(source_card, text="반기 정보 열이 없는 경우", style="Card.TLabel").grid(row=3, column=0, sticky="w", pady=(10, 0))
        semester_box = ttk.Combobox(source_card, textvariable=self.default_semester, values=("H1 (상반기)", "H2 (하반기)"), state="readonly", width=18)
        semester_box.grid(row=3, column=1, sticky="w", padx=(12, 8), pady=(10, 0))
        ttk.Button(source_card, text="TEa 기준 설정", command=self._open_tea_editor).grid(row=3, column=2, sticky="w", pady=(10, 0))
        ...
        ttk.Label(source_card, text="반기 열이 있으면 해당 열을 우선 사용합니다. 한 연도에 6개 물질이면 코드 순서대로 3개씩 H1/H2로 자동 분리합니다.", style="Muted.TLabel").grid(row=4, column=0, columnspan=4, sticky="w", pady=(13, 0))
        ...
        self.summary_tree = self._add_table_tab(notebook, "반기별 확인")
        self.subpeer_tree = self._add_table_tab(notebook, "Sub-peer pooled")
        self.peer_tree = self._add_table_tab(notebook, "Peer pooled")
        self.analyte_tree = self._add_table_tab(notebook, "Analyte pooled")
    ...
    def _semester_code(self) -> str:
        return "H2" if self.default_semester.get().startswith("H2") else "H1"
    ...
    def _run_analysis(self) -> None:
        ...
        self.status.set("Raw data를 확인하고 반기별 3-PT Harmonization을 계산하고 있습니다…")
        self._clear_tables()

        def work() -> None:
            try:
                result = analyze_workbook(source, destination, tea_map=self.tea_map, default_semester=self._semester_code())
                self.after(0, lambda: self._show_result(result))
            except Exception as exc:  # surfaced on the main Tk thread
    ...
    def _show_result(self, result: AnalysisResult) -> None:
        ...
        self._fill_tree(self.summary_tree, result.coverage)
        self._fill_tree(self.subpeer_tree, result.subpeer)
        self._fill_tree(self.peer_tree, result.peer)
        self._fill_tree(self.analyte_tree, result.analyte)
        self.status.set(f"완료: {', '.join(result.periods)} | PDF, Excel 표, 추세 그래프가 저장되었습니다. 결과 폴더: {result.pdf_path.parent}")
        messagebox.showinfo("분석 완료", f"반기별 Harmonization 평가가 완료되었습니다.\n\nPDF: {result.pdf_path}\nExcel: {result.tables_path}\nCharts: {result.charts_dir}")

    def _show_error(self, detail: str) -> None:
        self.progress.stop()
        self.run_button.configure(state="normal")
        self.status.set("분석을 완료하지 못했습니다. 파일 형식과 3-PT 물질 구성을 확인하세요.")
    ...
    def _fill_tree(self, tree: ttk.Treeview, frame: pd.DataFrame) -> None:
        display = frame.copy()
        if "evaluationPeriod" in display:
            display = display.sort_values(["evaluationPeriod", "analyte"], kind="stable")
        display = display.head(1000)
        ...
        for _, row in display.iterrows():
            ...
            level = str(row.get("harmonizationLevel", "")).lower().replace(" ", "_")
    ...
    def _open_tea_editor(self) -> None:
        ...
        for col, label in enumerate(("Optimal", "Desirable", "Minimum"), start=1):
            ttk.Label(container, text=label, style="Card.TLabel").grid(row=1, column=col, padx=5, pady=(14, 4))
        ...
        def add_row(name: str = "", threshold: TEaThreshold | None = None) -> None:
            ...
            for index, entry in enumerate(entries, start=1):
                entry.grid(row=row, column=index, padx=5, pady=3)
